      ...
      <Tab.Group>
        <Tab.List className="flex justify-center items-center mb-4">
          {categories.map((category) => (
            <Tab
              key={category}
              className={({ selected }) =>`text-xs md:text-sm px-2 py-1  border  ml-4 rounded-md  hover:text-indigo-900 mb-2 md:mb-0 ${selected ? 'text-indigo-900 border-indigo-900 hover:border-indigo-900' : 'text-gray-900 border-gray-900 hover:border-gray-900'}` }>
              {category}
            </Tab>
          ))}
        </Tab.List>
